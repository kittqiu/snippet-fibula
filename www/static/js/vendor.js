...
var ONEDAY = 3600*24*1000,
	WORKHOUR = 8,
	WORKTIME = 3600*WORKHOUR*1000,
	ZONEOFFSET = (new Date().getTimezoneOffset())*60000;
function wd_formatStart(millisecond){
	var n = millisecond - ZONEOFFSET;
	n = n - (n%ONEDAY);
	n += ZONEOFFSET;
	return n;
}
function wd_formatEnd(millisecond){
	var n = millisecond - ZONEOFFSET;
	n = n - (n%ONEDAY) + WORKTIME;
	n += ZONEOFFSET;
	return n;
}
function wd_after(start, duration){
	duration = duration || 0;
	var days = Math.floor(duration / WORKHOUR),
		hour = duration % WORKHOUR,
		end = start - ZONEOFFSET;

	end = end + hour * 3600000;
	if( end%ONEDAY > WORKTIME){//next day
		var remainder = end%ONEDAY - WORKTIME;
		end = (end + ONEDAY) - (end + ONEDAY)%ONEDAY ;
		end += remainder;
	}
	if( end%ONEDAY === 0 ){
		if( days >= 1 ){
