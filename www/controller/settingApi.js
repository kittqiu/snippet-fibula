...
/* simplify array defaultSettingDefinitions, only store key-value, drop label, description, type properties*/
...
function* $setSetting(key, value){
    ...
    group = m[1];
    yield warp.$update( 'delete from setting where `key`=?', [key] );
...
function* $setSettings(group, settings){
    yield warp.$update( 'delete from setting where `group`=?', group );
    var key;
    for( key in settings){
